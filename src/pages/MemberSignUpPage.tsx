...
const lbl: React.CSSProperties = { color: 'rgba(233,210,255,0.78)', fontSize: '12px', display: 'block', marginBottom: '6px', letterSpacing: '0.08em', textTransform: 'uppercase' }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!birthday) return setError('Please enter your birthday.')
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'radial-gradient(circle at top, rgba(191,0,255,0.22) 0%, rgba(139,92,246,0.14) 16%, rgba(30,0,40,0.9) 42%, #05010a 72%, #000000 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '32px 16px' }}>
      <div style={{ width: '100%', maxWidth: '430px' }}>
        <a href="/signin" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', color: 'rgba(221,171,255,0.6)', fontSize: '14px', textDecoration: 'none', marginBottom: '20px' }}>
          ← Back to sign in
        </a>

        <NoctuHeader subtitle="Create your free account" />

        <div style={{ background: 'rgba(16,0,26,0.78)', border: '1px solid rgba(191,0,255,0.18)', borderRadius: '20px', padding: '28px 22px', backdropFilter: 'blur(12px)', boxShadow: '0 24px 80px rgba(0,0,0,0.5)' }}>
          <div style={{ background: 'rgba(191,0,255,0.12)', border: '1px solid rgba(191,0,255,0.3)', borderRadius: '10px', padding: '10px 14px', marginBottom: '20px', textAlign: 'center' }}>
            <span style={{ color: '#d946ef', fontWeight: 700, fontSize: '13px' }}>FREE TO JOIN</span>
            <span style={{ color: 'rgba(255,255,255,0.55)', fontSize: '13px' }}> — upgrade anytime for exclusive perks</span>
          </div>

          {error && (
            <div style={{ background: 'rgba(90,0,40,0.45)', border: '1px solid rgba(255,70,140,0.55)', color: '#ffd7ef', padding: '12px 14px', borderRadius: '10px', marginBottom: '16px', fontSize: '14px' }}>
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <div>
              ...
              <input type="text" value={fullName} onChange={e => setFullName(e.target.value)} required placeholder="Your name" style={inp} />
            </div>
          ...
          <p style={{ color: 'rgba(255,255,255,0.45)', fontSize: '14px', textAlign: 'center', marginTop: '20px' }}>
            Already have an account? <a href="/signin" style={{ color: '#d946ef', textDecoration: 'none', fontWeight: 600 }}>Sign in</a>
          </p>
        </div>
      </div>
    </div>
